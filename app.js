...
const C = [
  {
    ...
    run: v => {
      const d = v.sell - v.cost;
      if (v.cost === 0) return "Cost price must not be zero.";
      return d >= 0
        ? `Profit: ${fmt(d)} | ${fmt(d / v.cost * 100)}%`
        : `Loss: ${fmt(Math.abs(d))} | ${fmt(Math.abs(d) / v.cost * 100)}%`;
    }
  },
  {
    id: "percentage",
  ...
  },
  {
    ...
    run: v => {
      const i = v.principal * v.rate * v.years / 100;
      return `Interest: ${fmt(i)} | Total: ${fmt(v.principal + i)}`;
    }
  },
  {
    id: "compound",
  ...
  },
  {
    ...
    run: v => {
      ...
      const emi = r === 0
        ...
        : v.principal * r * Math.pow(1 + r, v.months) /
  ...
  },
  {
    ...
    run: v => {
      const bill = v.area * (1 + v.waste / 100);
      return `Estimated Cost: ${fmt(bill * v.rate)} | Billable Area: ${fmt(bill)}`;
    }
  },
  {
    id: "temp",
...
function t(k) {
  return (T[state.lang] || T.en)[k] || T.en[k];
}

function renderTabs(q = "") {
  const list = C.filter(c =>
    c.name.toLowerCase().includes(q.toLowerCase())
  );

  tabs.innerHTML = list.map((c, i) =>
    `<button type="button" class="tab ${i === 0 ? "active" : ""}" data-id="${c.id}">${c.icon} ${c.name}</button>`
  ).join("") || `<span class="hint">No calculator found.</span>`;

  tabs.querySelectorAll(".tab").forEach(b =>
    b.addEventListener("click", () => openCalc(b.dataset.id))
  );
}
...
function openCalc(id) {
  ...
  app.innerHTML = `
    <article class="card">
      <h2>${c.icon} ${c.name}</h2>
      <p class="hint">Enter values and calculate instantly.</p>
      <form id="calcForm">
        <div class="fields">
          ${c.fields.map(([k, l]) => `
            <div class="field">
              <label for="${id}-${k}">${l}</label>
              <input id="${id}-${k}" name="${k}" type="number" inputmode="decimal" step="any" placeholder="0" required>
            </div>
          `).join("")}
        </div>

        <div class="actions">
          <button class="btn primary" type="submit">${t("calculate")}</button>
          <button class="btn" id="reset" type="button">${t("reset")}</button>
        </div>
  ...
  form.addEventListener("submit", e => {
    ...
    for (const [k] of c.fields) {
      const n = Number(form.elements[k].value);

      if (!Number.isFinite(n)) {
        result.hidden = false;
        result.innerHTML = `<div class="error">${t("invalid")}</div>`;
        return;
      }
    ...
    try {
      ...
      out = "Calculation error. Please check your inputs.";
    }
    ...
    result.innerHTML = `
      ...
      <strong>${esc(out)}</strong>
      <div class="actions">
        <button class="btn" id="copy" type="button">${t("copy")}</button>
      </div>
    `;
...
function openHistory() {
  app.innerHTML = `
    <article class="card">
      ...
      <div class="actions">
        <button id="clearHistory" class="btn danger" type="button">${t("clear")}</button>
      </div>
...
document.querySelectorAll(".footer a").forEach(a =>
  a.addEventListener("click", e => {
    ...
    const text = {
      privacy: [
        "Privacy Policy",
        "SmartCalculator is designed to perform calculations in your browser. We do not intentionally collect calculator inputs. If third-party analytics or advertising is added later, this policy should be updated before launch."
      ],
      terms: [
        "Terms of Use",
        "SmartCalculator provides calculations for general informational purposes. Verify important financial, tax, construction or business calculations with an appropriate professional."
      ],
      disclaimer: [
        "Disclaimer",
        "Calculator results may contain rounding or input errors. SmartCalculator does not provide financial, legal, tax, engineering or medical advice."
      ]
    }[id];
